...
または圧縮形式を **Zip（パスワード）** に設定している **CubeICE 圧縮** ショートカットにファイルやフォルダーをドラッグ&amp;ドロップした場合、
...
* **指定したフォルダー**  
  常にユーザーが指定したフォルダーに圧縮ファイルを作成します。
  尚、**指定したフォルダー** をチェックしただけ（フォルダーの指定なし）の場合、デスクトップに圧縮ファイルを作成します。
* **元のファイルと同じフォルダー**  
  圧縮対象となるファイルやフォルダーと同じフォルダーに圧縮ファイルも作成します。
* **実⾏時に指定する**  
...
* **圧縮後に保存先フォルダーを開く**  
  圧縮処理が終了した後に、作成された圧縮ファイルが保存されているフォルダーを⾃動的に開きます。
...
* **指定したフォルダー**  
  常にユーザーが指定したフォルダーに圧縮ファイルを解凍します。
  **指定したフォルダー** をチェックしただけ（フォルダーの指定なし）の場合、デスクトップに圧縮ファイルを解凍します。
* **元のファイルと同じフォルダー**  
  圧縮ファイルと同じフォルダーに解凍します。
* **実⾏時に指定する**  
...
* **フォルダーを自動的に作成する**  
  解凍する際に、元の圧縮ファイル名のフォルダーを⾃動的に作成し、そのフォルダーの中に解凍したファイル、フォルダーを保存します。
  この設定には、指定された圧縮ファイルが単一のフォルダーに全てのファイルが格納されている場合、フォルダーを作成しないようにするサブ項目が存在します。
* **ファイルのフィルタリングを⾏う**  
  解凍する際に、特定の条件に合致する名前のファイルを保存しないようにします。
  保存しない名前のファイルの設定は、**詳細** タブで設定可能です。
* **圧縮後に保存先フォルダーを開く**  
  解凍処理が終了した後に、保存フォルダーを⾃動的に開きます。
  この設定には、**デスクトップの場合は開かない** ように設定できるサブ項目が存在します。
* **解凍後に元のファイルを削除する**  
